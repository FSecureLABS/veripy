...
class NonUniqueSourceUDPPortUnreachableTestCase(ComplianceTestCase):
    """
    Error Condition With Non-Unique Source - UDP Port Unreachable
...
class NonUniqueSourceEchoRequestTooBigTestCase(ComplianceTestCase):
    """
    Error Condition With Non-Unique - Echo Request Too Big
    ...
    def run(self):
        ...
        self.node(1).send( \
            util.pad( \
                IPv6(src="::", dst=str(self.node(4).global_ip()))/
...
class NonUniqueSourceReassemblyTimeoutTestCase(ComplianceTestCase):
    """
    Error Condition With Non-Unique Source - Echo Request Reassembly Timeout
...
class NonUniqueSourceInvalidDestinationOptionsTestCase(ComplianceTestCase):
    """
    Error Condition With Non-Unique - Echo Request with Unknown Option in
    Destination Options
    ...
    def run(self):
        ...
        assertEqual(0, len(r1), "did not expect to receive an ICMPv6 Parameter Problem")
