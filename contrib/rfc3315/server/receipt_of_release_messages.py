...
class ReceiptOfReleaseMessagesTestCase(DHCPv6Helper):
    """
    Receipt of Release Messages

    Verify a client and server device properly handles Release messages.
    ...
    def run(self):
        ...
        self.logger.info("Building a DHCPv6 Release message to send to the server...")
        l = self.build_dhcpv6_release(p, self.target(1), self.node(1))

        self.node(1).clear_received()
        self.logger.info("Sending the DHCPv6 Release message to the server...")
        self.node(1).send( \
