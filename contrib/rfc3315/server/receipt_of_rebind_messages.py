...
class ReceiptOfRebindMessagesTestCase(DHCPv6Helper):
    """
    Receipt of Rebind Messages

    Verify a client and server device properly handles Rebind messages.
    ...
    def run(self):
        ...
        self.logger.info("Building a DHCPv6 Rebind message to send to the server...")
        n = self.build_dhcpv6_rebind(p, self.target(1), self.node(1))

        self.logger.info("Sending the DHCPv6 Rebind message to the server...")
        self.node(1).send( \
