...
class TransmissionOfRebindMessagesTestCase(DHCPv6Helper):
    """
    ...
    @private
    Source:         IPv6 Ready DHCPv6 Interoperability Test Suite (Section 1.4)
    """

    def run(self):
        self.do_dhcpv6_handshake_as_server(self.node(1), self.target(1), T1=50, T2=80)
    
        self.logger.info("Waiting 50s for the rebind timer to expire...")
        self.ui.wait(80)

        self.logger.info("Expecting the client to have sent a DHCPv6 Rebind message...")
        r1 = self.node(1).received(src=str(self.target(1).link_local_ip()), dst=AllDHCPv6RelayAgentsAndServers, type=DHCP6_Rebind)

        assertGreaterThanOrEqualTo(1, len(r1), "expected to receive a DHCPv6 Rebind message")

        n = r1[0]

        assertHasLayer(DHCP6OptIA_NA, n, "expected the DHCPv6 Rebnd message to have an IA")
        ...
        self.logger.info("Sending a DHCPv6 reply to the Rebind message...")
