...
class PathMTUOptionTestCase(RedirectionHelper):
    """
    Invalid Option - Path MTU Option

    Verify that a host ignores invalid options in Redirect messages and
    processes the remainder of the Redirect normally.

    @private
    Source:         IPv6 Ready Phase-1/Phase-2 Test Specification Core
                    Protocols (Test v6LC.2.3.8a)
    """
    ...
    def set_up(self):
        self.p = IPv6(src=str(self.router(1).link_local_ip(iface=1)), dst=str(self.target(1).link_local_ip()), hlim=255)/\
                    ICMPv6ND_Redirect(dst=str(self.node(2).global_ip()), tgt=str(self.node(2).global_ip()))/\
                        ICMPv6NDOptMTU(mtu=1280)


class PrefixInformationOptionTestCase(RedirectionHelper):
    """
    Invalid Option - Prefix Information Option
    ...
    @private
    Source:         IPv6 Ready Phase-1/Phase-2 Test Specification Core
                    Protocols (Test v6LC.2.3.8b)
    """

    def set_up(self):
        self.p = IPv6(src=str(self.router(1).link_local_ip(iface=1)), dst=str(self.target(1).link_local_ip()), hlim=255)/\
                    ICMPv6ND_Redirect(dst=str(self.node(2).global_ip()), tgt=str(self.node(2).global_ip()))/\
                        ICMPv6NDOptPrefixInfo(prefix="db8::", prefixlen=64)

class SourceLinkLayerAddressOption(RedirectionHelper):
    """
    Invalid Option - Source Link-layer Address Option
    ...
    @private
    Source:         IPv6 Ready Phase-1/Phase-2 Test Specification Core
                    Protocols (Test v6LC.2.3.8c)
    """

    def set_up(self):
        self.p = IPv6(src=str(self.router(1).link_local_ip(iface=1)), dst=str(self.target(1).link_local_ip()), hlim=255)/\
                    ICMPv6ND_Redirect(dst=str(self.node(2).global_ip()), tgt=str(self.node(2).global_ip()))/\
                        ICMPv6NDOptSrcLLAddr(lladdr=self.node(4).iface(0).ll_addr)
