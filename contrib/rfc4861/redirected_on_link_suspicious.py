...
class OptionUnrecognizedTestCase(RedirectionHelper):
    """
    Redirected On-link: Suspicious - Option Unrecognized
...
class ReservedFieldIsNonZeroTestCase(RedirectionHelper):
    """
    Redirected On-link: Suspicious - Reserved Field is Non-zero
    ...
    @private
    Source:         IPv6 Ready Phase-1/Phase-2 Test Specification Core
                    Protocols (Test v6LC.2.3.2b)
    """

    def set_up(self):
        self.p = IPv6(src=str(self.router(1).link_local_ip(iface=1)), dst=str(self.target(1).link_local_ip()), hlim=255)/\
                    ICMPv6ND_Redirect(dst=str(self.node(2).global_ip()), tgt=str(self.node(2).global_ip()), res=1)
