...
class ReleaseMessageTestCase(DHCPv6PDHelper):
    ...
    def run(self):
        ...
        self.logger.info("Waiting for a DHCPv6 Release message, with a IA for Prefix Delegation...")
        r3 = self.node(1).received(src=self.target(1).link_local_ip(), dst=AllDHCPv6RelayAgentsAndServers, type=DHCP6_Release)
        assertGreaterThanOrEqualTo(1, len(r3), "expected to receive one-or-more DHCPv6 Rebind messages")
        assertHasLayer(DHCP6OptIA_PD, r3[0], "expected the DHCPv6 Rebind to contain an IA for Prefix Delegation")
        assertHasLayer(DHCP6OptIAPrefix, r3[0], "expected the DHCPv6 Rebind to contain an IA Prefix")
        assertEqual(r2[0][DHCP6OptIAPrefix].prefix, r3[0][DHCP6OptIAPrefix].prefix, "expected the original prefix to be rebound")
