...
class RebindMessageTestCase(DHCPv6PDHelper):
    ...
    def run(self):
        ...
        self.logger.info("Waiting for a DHCPv6 Rebind message, with a IA for Prefix Delegation...")
        r3 = self.node(1).received(src=self.target(1).link_local_ip(), dst=AllDHCPv6RelayAgentsAndServers, type=DHCP6_Rebind)
